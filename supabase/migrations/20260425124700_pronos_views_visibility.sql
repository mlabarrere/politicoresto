-- Make pending / rejected pronos visible on their own page (so the
-- demandeur sees their own pending request) while keeping the home
-- feed clean. Updates `v_thread_detail` to include the two new topic
-- statuses, and tightens `v_feed_global` to filter them back out.
...
select t.id,
  ...
  case
    when t.topic_status = 'pending_review'::public.topic_status then 'pending_review'::text
    when t.topic_status = 'rejected'::public.topic_status then 'rejected'::text
...
where t.topic_status = any (array[
        'pending_review'::public.topic_status,
        'open'::public.topic_status,
        'locked'::public.topic_status,
        'resolved'::public.topic_status,
        'archived'::public.topic_status,
        'rejected'::public.topic_status])
...
-- Admin queue : pending pronos with their requester. Modo only via RLS
-- on the underlying topic + prono_question RLS (which already require
-- is_moderator to update). Service-role grants are kept for tooling.
create or replace view public.v_prono_admin_queue
...
select t.id as topic_id,
       ...
       (select count(*) from public.prono_option po where po.question_id = pq.id) as option_count
from public.topic t
join public.prono_question pq on pq.topic_id = t.id
join public.app_profile ap on ap.user_id = pq.requested_by
where t.topic_status = 'pending_review'::public.topic_status
  and public.is_moderator()
order by t.created_at asc;

alter view public.v_prono_admin_queue owner to postgres;
grant select on public.v_prono_admin_queue to authenticated, service_role;
