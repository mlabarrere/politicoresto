-- Drop the legacy `prediction_*` infrastructure that was scaffolded in the
-- baseline migration but never reached the frontend. The Pronostics feature
-- replaces it with a different model (`prono_*`, see follow-up migration);
-- keeping both schemas around would cohabit two unrelated lifecycles in
-- production. This migration is forward-only and idempotent.
--
-- Conserved on purpose:
--   * enum value `'market'` in `thread_post_type` — reused by Pronostics.
--   * enum value `'prediction_accuracy'` in `reputation_event_type` —
--     reused as scoring event for Pronostics (semantically identical).
--   * enum value `'prediction_submission'` in `moderation_target_type` —
--     audit-only reference, no compelling reason to rebuild the type.

-- 1. Functions (CASCADE drops the triggers that reference them)
drop function if exists public.compute_prediction_normalized_score(uuid, uuid) cascade;
...
drop function if exists public.rpc_create_topic_with_prediction(uuid, text, text, text, text, uuid, public.prediction_type, text, public.prediction_scoring_method, public.prediction_aggregation_method) cascade;

-- 2. Tables (CASCADE removes FK constraints and the editorial mirror)
...
drop table if exists public.prediction_questions_editorial cascade;

-- 3. Enums (CASCADE removes any straggler column references; columns are
--    dropped above so this is purely defensive).
...
-- 3b. Editorial mirror column for the deprecated feature.
alter table if exists public.topics_editorial drop column if exists is_prediction_enabled;

-- 4. Recreate `v_feed_global` and `v_thread_detail` without the legacy
--    `prediction_type`, `prediction_question_title`, `active_prediction_count`
--    columns. v_feed_global depends on v_thread_detail, so drop in order.
...
-- 5. Rewrite `create_post(uuid, text, ...)` overload to drop the legacy
--    `'market' -> prediction_question` branch. The new Pronostics RPC
--    `rpc_request_prono` will create the prono_question itself (lot 1).
