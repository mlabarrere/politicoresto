...
class TestCanonicalOrdering:
    def test_past_vote_is_highest_priority(self) -> None:
        ...
        assert CANONICAL_DIMS[0] == "past_vote_presidentielle_2022_t1"
    ...
    def test_demographic_dims_all_present(self) -> None:
        ...
        assert set(CANONICAL_DIMS) >= expected | {"past_vote_presidentielle_2022_t1"}
...
class TestPickCalibrationDims:
    def test_small_panel_keeps_only_highest_priority(self) -> None:
        ...
        available = {"age_bucket", "sex", "past_vote_presidentielle_2022_t1"}
        ...
        # priority). past_vote_presidentielle_2022_t1 comes first in CANONICAL_DIMS.
        assert selected == ["past_vote_presidentielle_2022_t1"]

    def test_larger_panel_includes_multiple(self) -> None:
        """n=100, budget=10 → top-10 of the canonical list."""
        available = set(CANONICAL_DIMS)
        selected = pick_calibration_dims(
            n_respondents=100, available_in_reference=available
        )
        assert len(selected) == 10
        assert selected == list(CANONICAL_DIMS[:10])

    def test_huge_panel_includes_everything(self) -> None:
        """n=10000, budget=1000 → every canonical dim available."""
        available = set(CANONICAL_DIMS)
        selected = pick_calibration_dims(
            n_respondents=10_000, available_in_reference=available
        )
    ...
    def test_missing_dim_is_skipped(self) -> None:
        ...
        assert "past_vote_presidentielle_2022_t1" not in selected
    ...
    def test_respects_priority_order_in_output(self) -> None:
        ...
        available = {"education", "sex", "past_vote_presidentielle_2022_t1"}
        selected = pick_calibration_dims(
            n_respondents=200, available_in_reference=available
        )
        # past_vote first (priority 0), sex (priority 2), education (priority 5)
        assert selected.index("past_vote_presidentielle_2022_t1") == 0
        assert selected.index("sex") < selected.index("education")

    def test_tight_budget_drops_low_priority_first(self) -> None:
        """n=20, budget=2 → keep top-2 of priority list (both past-vote)."""
        available = set(CANONICAL_DIMS)
        selected = pick_calibration_dims(
            n_respondents=20, available_in_reference=available
        )
        # budget = 20 // 10 = 2. Top-2 priority are past-vote dims.
        assert len(selected) == 2
        assert selected[0] == "past_vote_presidentielle_2022_t1"
        # Demographic dims dropped when budget is tight.
        assert "age_bucket" not in selected
    ...
    def test_zero_respondents_still_returns_one_dim_when_possible(
        ...
        available = {"past_vote_presidentielle_2022_t1"}
        selected = pick_calibration_dims(
