...
test.describe('auth on staging (real Supabase)', () => {
  ...
  test('signInWithPassword persists session cookie + /me is authenticated', async ({
    page,
    context,
    request,
  }) => {
    ...
    expect(
      resp?.status(),
      'page /me should load directly, not redirect to login',
    ).toBe(200);
    ...
    expect(
      authCookie,
      `${cookieName} cookie must persist after /me render`,
    ).toBeTruthy();
  ...
  test('unauthenticated access to /me redirects to /auth/login', async ({
    page,
  }) => {
    const resp = await page.goto(`${BASE_URL}/me`);
