...
function makeClient({
  ...
  return {
    auth: {
      getClaims: async () => ({
        data: { claims: userNull ? null : { sub: userId } }
      }),
      getSession: async () => ({
        data: { session: userNull ? null : { user: { id: userId } } }
      })
    },
    from: fromMock,
...
describe("POST /api/reactions", () => {
  ...
  it("returns 401 when not authenticated", async () => {
    ...
    const response = await POST(makeRequest({ targetType: "post", targetId: "x", side: "gauche" }));
    expect(response.status).toBe(401);
  });

  it("returns 400 when payload is invalid", async () => {
    mocks.createServerSupabaseClient.mockResolvedValue(makeClient());
    const response = await POST(makeRequest({ targetType: "invalid", targetId: "", side: "gauche" }));
  ...
  it("handles comment targetType", async () => {
    mocks.createServerSupabaseClient.mockResolvedValue(makeClient());
    const response = await POST(
      makeRequest({ targetType: "comment", targetId: "comment-1", side: "droite" })
    );
    expect(response.status).toBe(200);
  });
  ...
  it("returns 400 when counts read fails", async () => {
    mocks.createServerSupabaseClient.mockResolvedValue(
      makeClient({ countsError: { message: "counts error", code: "500" }, countsData: null })
    );
    const response = await POST(
      makeRequest({ targetType: "post", targetId: "post-1", side: "gauche" })
    );
