...
function makeReactionQuery(
  rows: { target_id: string; reaction_type: string }[],
) {
  return {
    select: vi.fn(() => ({
      eq: vi.fn(() => ({
  ...
  return {
    from: vi.fn((table: string) => {
      if (table === 'v_thread_detail')
        return {
          select: vi.fn(() => makeSelectQuery({ data: topic, error: null })),
        };
      if (table === 'v_thread_posts')
        return {
          select: vi.fn(() => makeSelectQuery({ data: posts, error: null })),
        };
      if (table === 'v_post_comments')
        return {
          select: vi.fn(() => makeSelectQuery({ data: comments, error: null })),
        };
    ...
    mockedCreateServerSupabaseClient.mockResolvedValue({
      from: vi.fn((table: string) => {
        ...
        if (table === 'v_post_comments')
          return {
            select: vi.fn(() =>
              makeSelectQuery({ data: comments, error: null }),
            ),
          };
        if (table === 'v_post_poll_summary')
          return {
  ...
  it('returns null for unknown slug', async () => {
    mockedCreateServerSupabaseClient.mockResolvedValue({
      from: vi.fn((table: string) => {
        if (table === 'v_thread_detail')
          return {
            select: vi.fn(() => makeSelectQuery({ data: null, error: null })),
          };
        throw new Error(`Unexpected table ${table}`);
      }),
    } as never);
