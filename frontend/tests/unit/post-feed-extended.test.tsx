...
vi.mock("@/components/app/app-feed-item", () => ({
  AppFeedItem: ({ item }: { item: { topic_id: string; topic_title: string } }) => (
...
describe("PostFeed", () => {
  ...
  it("filters sondage open correctly", () => {
    ...
    const withoutPoll = buildHomeFeedTopic({ topic_id: "t2", feed_poll_summary: undefined });
  ...
  it("filters politique by bloc slug", () => {
    const lfiItem = buildHomeFeedTopic({ topic_id: "t1", primary_taxonomy_slug: "lfi" });
    const rnItem = buildHomeFeedTopic({ topic_id: "t2", primary_taxonomy_slug: "rn" });
  ...
  it("sorts by recent when sortMode=recent", () => {
    const older = buildHomeFeedTopic({ topic_id: "t-old", last_activity_at: "2026-01-01T00:00:00Z" });
    const newer = buildHomeFeedTopic({ topic_id: "t-new", last_activity_at: "2026-04-01T00:00:00Z" });

    render(
      <PostFeed items={[older, newer]} isAuthenticated={false} sortMode="recent" />
    );
