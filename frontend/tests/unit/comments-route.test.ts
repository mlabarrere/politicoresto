...
function makeClient({
  ...
  return {
    auth: makeAuthMock(userNull ? null : 'user-1'),
    from: fromMock,
...
describe('POST /api/comments', () => {
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    const response = await POST(
      makeRequest({ postSlug: 'slug', body: 'hello' }),
    );
  ...
  it('returns 400 when post not found by slug', async () => {
    mocks.createServerSupabaseClient.mockResolvedValue(
      makeClient({ postSlugError: true }),
    );
    const response = await POST(
      makeRequest({ postSlug: 'bad-slug', body: 'hello' }),
    );
    expect(response.status).toBe(400);
  });
  ...
  it('returns 400 when rpc create_comment fails', async () => {
    ...
    const response = await POST(
      makeRequest({ postSlug: 'valid-slug', body: 'hello' }),
    );
    expect(response.status).toBe(400);
  });
...
describe('PATCH /api/comments', () => {
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    const response = await PATCH(
      makeRequest({ commentId: 'c1', body: 'updated' }, 'PATCH'),
    );
    expect(response.status).toBe(401);
  });
