...
describe("AuthCodeErrorPage", () => {
  it("renders oauth_missing_code message", async () => {
    ...
    expect(screen.getByRole("heading", { name: /Erreur d.authentification/i })).toBeInTheDocument();
  ...
  it("retry link includes next param when provided", async () => {
    render(
      await AuthCodeErrorPage({
        searchParams: Promise.resolve({ reason: "oauth_missing_code", next: "/post/abc" })
      })
    );
