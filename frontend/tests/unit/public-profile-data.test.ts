          ...
          select: vi.fn().mockReturnValue({
            eq: vi.fn().mockReturnValue({
              order: vi.fn().mockReturnValue({
                limit: vi.fn().mockResolvedValue({ data: posts, error: postsError })
              })
            })
          })
        };
      ...
      if (table === "v_post_comments") {
        return {
          select: vi.fn().mockReturnValue({
            eq: vi.fn().mockReturnValue({
              order: vi.fn().mockReturnValue({
                limit: vi.fn().mockResolvedValue({ data: comments, error: commentsError })
              })
            })
          })
        };
...
describe("getPublicProfile", () => {
  ...
  it("returns null when profile not found", async () => {
    mocks.createServerSupabaseClient.mockResolvedValue(makeClient({ profile: null as never }));
  ...
  it("throws when profile fetch errors", async () => {
    mocks.createServerSupabaseClient.mockResolvedValue(
      makeClient({ profile: null as never, profileError: { message: "db error" } })
    );
    await expect(getPublicProfile("citoyen")).rejects.toEqual({ message: "db error" });
  });
  ...
  it("throws when posts fetch errors", async () => {
    ...
    await expect(getPublicProfile("citoyen")).rejects.toEqual({ message: "posts error" });
  });
  ...
  it("throws when comments fetch errors", async () => {
    ...
    await expect(getPublicProfile("citoyen")).rejects.toEqual({ message: "comments error" });
  });

  it("maps posts with thread slugs when topics available", async () => {
    const posts = [{ id: "p1", thread_id: "t1", title: "Mon post", content: "Corps", created_at: "2026-01-01" }];
  ...
  it("maps comments with thread slugs", async () => {
    const comments = [{ id: "c1", thread_id: "t1", body_markdown: "Mon commentaire", created_at: "2026-01-01" }];
