...
describe("lib/logger", () => {
  it("rootLogger exposes the configured level", () => {
    ...
    expect(["trace", "debug", "info", "warn", "error", "fatal"]).toContain(rootLogger.level);
  });
  ...
  it("withUser binds user_id and is opt-in for user_email", () => {
    ...
    const withEmail = withUser(createLogger("test.user"), { id: "u-2", email: "a@b.c" });
  ...
  it("withUser ignores a null email", () => {
    const log = withUser(createLogger("test.user2"), { id: "u-3", email: null });
