...
function makeClient(opts: {
  ...
  const electionQuery = {
    ...
    then: (resolve: (r: QueryResult) => void) => Promise.resolve(opts.elections).then(resolve)
  };
  const resultQuery = {
    select: vi.fn().mockReturnThis(),
    order: vi.fn().mockReturnThis(),
    then: (resolve: (r: QueryResult) => void) => Promise.resolve(opts.results).then(resolve)
  };
  return {
    ...
    rpc: vi.fn((name: string) => {
      if (name === "rpc_list_vote_history_detailed") return Promise.resolve(opts.votes);
...
describe("getVoteHistoryEditorData", () => {
  ...
  it("returns status=ready with merged elections + results + votes", async () => {
    mocks.createServerSupabaseClient.mockResolvedValue(
      makeClient({
        elections: {
          data: [
            { id: "e1", slug: "pres-2022-t1", type: "presidentielle", year: 2022, round: 1, held_on: "2022-04-10", label: "Pres 2022 T1" }
          ],
          error: null
        },
        results: {
          data: [
            { id: "r1", election_id: "e1", rank: 1, candidate_name: "A", list_label: null, party_slug: "renaissance", nuance: "ENS", pct_exprimes: 27.85 }
          ],
          error: null
        },
  ...
  it("maps a user's vote into votesByElectionId by election_id", async () => {
    mocks.createServerSupabaseClient.mockResolvedValue(
      makeClient({
        elections: { data: [{ id: "e1", slug: "pres-2022-t1", type: "presidentielle", year: 2022, round: 1, held_on: "2022-04-10", label: "L" }], error: null },
  ...
  it("reports unavailable when election table is missing (capability missing)", async () => {
    mocks.createServerSupabaseClient.mockResolvedValue(
      makeClient({
        elections: { data: null, error: { message: "schema cache miss", code: "PGRST202" } },
        results: { data: [], error: null },
        votes: { data: [], error: null }
      })
    );
  ...
  it("still returns elections when the votes RPC fails (soft-fail)", async () => {
    mocks.createServerSupabaseClient.mockResolvedValue(
      makeClient({
        elections: { data: [{ id: "e1", slug: "s", type: "presidentielle", year: 2022, round: 1, held_on: "2022-04-10", label: "L" }], error: null },
        results: { data: [], error: null },
