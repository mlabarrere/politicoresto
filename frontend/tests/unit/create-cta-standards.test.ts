...
describe('Create CTA standards', () => {
  ...
  it('uses AppPrimaryCTA in global header', () => {
    const source = fs.readFileSync(
      path.join(root, 'components/layout/app-header.tsx'),
      'utf8',
    );
  ...
  it('removes legacy create entrypoints', () => {
    const homeShell = fs.readFileSync(
      path.join(root, 'components/home/homepage-shell.tsx'),
      'utf8',
    );

    expect(homeShell).not.toContain('CreatePostCTA');
    expect(
      fs.existsSync(path.join(root, 'components/home/right-sidebar.tsx')),
    ).toBe(false);
    expect(
      fs.existsSync(path.join(root, 'components/home/create-post-cta.tsx')),
    ).toBe(false);
  });
