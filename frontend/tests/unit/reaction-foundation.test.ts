...
describe('political reaction anti-spam foundation', () => {
  it('react_post toggle contract: same reaction type removes it, different type replaces it', () => {
    ...
    const toggleOffBehavior =
      'if existing_row.reaction_type = p_reaction_type then delete';
    expect(toggleOffBehavior).toContain(
      'existing_row.reaction_type = p_reaction_type',
    );
  ...
  it('react_post update contract: different reaction type updates in-place', () => {
    ...
    const updateBehavior =
      'update public.reaction set reaction_type = p_reaction_type';
