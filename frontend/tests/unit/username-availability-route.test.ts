...
function makeRequest(value: string) {
  return new Request(`http://localhost/api/account/username-availability?value=${encodeURIComponent(value)}`);
}
...
describe("GET /api/account/username-availability", () => {
  ...
  it("returns available=false when username is taken by another user", async () => {
    mocks.createServerSupabaseClient.mockResolvedValue(
      makeClient({ currentUsername: "my_name", duplicateData: { user_id: "other-user" } })
    );
