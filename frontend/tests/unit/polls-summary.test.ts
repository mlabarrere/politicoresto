...
describe("normalizePostPollSummary", () => {
  ...
  it("maps poll_status closed correctly", () => {
    const result = normalizePostPollSummary({ ...baseRow, poll_status: "closed" });
    expect(result?.poll_status).toBe("closed");
  });

  it("defaults unknown poll_status to open", () => {
    const result = normalizePostPollSummary({ ...baseRow, poll_status: "unknown" });
    expect(result?.poll_status).toBe("open");
  });
  ...
  it("normalizes raw_results with share and counts", () => {
    const row = {
      ...baseRow,
      raw_results: [
        { option_id: "opt-1", option_label: "Oui", sort_order: 0, share: 0.6, response_count: 60, weighted_count: 58 }
      ]
    };
    const result = normalizePostPollSummary(row);
  ...
  it("handles selected_option_id string", () => {
    const result = normalizePostPollSummary({ ...baseRow, selected_option_id: "opt-2" });
    expect(result?.selected_option_id).toBe("opt-2");
  });

  it("sets selected_option_id to null for non-string value", () => {
    const result = normalizePostPollSummary({ ...baseRow, selected_option_id: 42 });
    expect(result?.selected_option_id).toBeNull();
  });

  it("handles empty arrays gracefully", () => {
    const result = normalizePostPollSummary({ ...baseRow, options: [], raw_results: [], corrected_results: [] });
  ...
  it("handles non-array options field", () => {
    const result = normalizePostPollSummary({ ...baseRow, options: "not-an-array" });
    expect(result?.options).toHaveLength(0);
  });
});
