...
describe('post page forum UX', () => {
  ...
  it('renders OP and comments with normalized line breaks and strict forum order', async () => {
    ...
    render(
      await PostDetailPage({ params: Promise.resolve({ slug: 'post-1' }) }),
    );

    expect(
      screen.getAllByText(
        (content) => content.includes('Ligne 1') && content.includes('Ligne 2'),
      ).length,
    ).toBeGreaterThan(0);
    expect(screen.queryByText(/\\n/)).not.toBeInTheDocument();
    expect(
      screen.getByRole('article', { name: 'Post principal' }),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getAllByLabelText("C'est de gauche !").length,
    ).toBeGreaterThan(0);
    expect(
      screen.getAllByLabelText("C'est de droite !").length,
    ).toBeGreaterThan(0);
    expect(
      screen.getByText(
        (content) =>
          content.includes('Commentaire parent') &&
          content.includes('avec retour'),
      ),
    ).toBeInTheDocument();
  ...
  it('opens auth sheet for logged-out reactions without post load error', async () => {
    ...
    expect(
      screen.queryByText("Le thread n'a pas pu etre charge"),
    ).not.toBeInTheDocument();
  });
  ...
  it('renders explicit forbidden state when backend denies access', async () => {
    ...
    render(
      await PostDetailPage({
        params: Promise.resolve({ slug: 'post-locked' }),
      }),
    );

    expect(screen.getByText('Acces refuse')).toBeInTheDocument();
    expect(
      screen.getByText(/n'est pas accessible avec vos droits/i),
    ).toBeInTheDocument();
  });
});
