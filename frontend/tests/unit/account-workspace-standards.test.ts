...
describe('Account workspace standards', () => {
  ...
  it('renders one section nav per breakpoint in account shell', () => {
    const source = fs.readFileSync(
      path.join(root, 'components/app/app-account-shell.tsx'),
      'utf8',
    );
  ...
  it('keeps /me/settings as redirect to unified account workspace', () => {
    const source = fs.readFileSync(
      path.join(root, 'app/(authenticated)/me/settings/page.tsx'),
      'utf8',
    );
  ...
  it('uses AppPrimaryCTA global and removes legacy account shell files', () => {
    expect(
      fs.existsSync(
        path.join(root, 'components/layout/authenticated-shell.tsx'),
      ),
    ).toBe(false);
    expect(
      fs.existsSync(path.join(root, 'components/navigation/auth-nav.tsx')),
    ).toBe(false);
  });
});
