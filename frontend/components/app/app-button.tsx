...
export function AppButton({
    ...
    return (
      <Link
        ...
        className={cn(
          ...
          size === "sm" ? "px-3 py-1.5 text-xs" : size === "lg" ? "px-4 py-2.5 text-sm" : "px-3.5 py-2 text-sm",
          variant === "primary" && "border-primary bg-primary text-primary-foreground hover:brightness-95",
          variant === "secondary" && "border-border bg-secondary text-secondary-foreground hover:brightness-95",
  ...
  return (
    <CatalystButton tone={toneByVariant[variant]} size={size} icon={icon} className={className} {...props}>
      {children}
    </CatalystButton>
  );
}
