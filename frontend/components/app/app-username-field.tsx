...
export function AppUsernameField({
  ...
  const [message, setMessage] = useState<string>("3 a 24 caracteres, minuscules, chiffres et underscore.");
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const response = await fetch(`/api/account/username-availability?value=${encodeURIComponent(normalized)}`, {
          method: "GET",
          cache: "no-store"
        });
        ...
        if (payload.available || payload.isCurrentUsername) {
          setState("available");
          setMessage(payload.isCurrentUsername ? "Votre username actuel." : "Username disponible.");
          return;
        }
  ...
  return (
    <label className="block space-y-2">
      <span className="text-xs font-medium text-muted-foreground">Username unique</span>
      <AppInput
        name="username"
        required
        value={value}
