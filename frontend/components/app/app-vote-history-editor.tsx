...
const ABSTENTION_OPTIONS: Array<{
...
export function AppVoteHistoryEditor({
  ...
  return (
    <div className="space-y-4">
        {error ? (
          <AppBanner title="Enregistrement impossible" body={error} tone="warning" />
        ) : null}
          ...
          <AppCard key={group.key} className="space-y-3 p-4">
            ...
            <div className="space-y-3">
              {group.elections.map((election) => {
                const currentVote = votesByElectionId[election.id];
                  ...
                  <div key={election.id} className="space-y-2">
                    ...
                    <div className="grid grid-cols-[repeat(auto-fill,minmax(72px,1fr))] gap-2">
                      {election.results.map((result) => {
                        ...
                        const key = `${election.slug}:${result.id}`;
                        const isLoading = isPending && pendingKey === key;
                        const theme = getPartyTheme(result.party_slug);
                        const displayName =
                          result.candidate_name ?? result.list_label ?? "Candidat";
                        const tooltipLabel = result.candidate_name
                        ...
                        return (
                              <button
                                key={result.id}
                                type="button"
                                onClick={() => handleResultClick(election, result.id)}
                                disabled={isPending}
                                aria-pressed={isSelected}
                                aria-label={`${tooltipLabel} — ${isSelected ? "selectionne" : "non selectionne"}`}
                                title={tooltipLabel + (result.pct_exprimes != null ? ` (${result.pct_exprimes.toFixed(2).replace(".", ",")}%)` : "")}
                                style={
                                  isSelected
                                    ? {
                      ...
                      {ABSTENTION_OPTIONS.map((opt) => {
                        const isSelected = currentVote?.choice_kind === opt.kind;
                        const key = `${election.slug}:${opt.kind}`;
                        const isLoading = isPending && pendingKey === key;
                        return (
                              <button
                                key={opt.kind}
                                type="button"
                                onClick={() => handleAbstentionClick(election, opt.kind)}
                                disabled={isPending}
                                aria-pressed={isSelected}
                                aria-label={`${opt.label} — ${isSelected ? "selectionne" : "non selectionne"}`}
                                title={opt.label}
                                style={
                                  isSelected
                                    ? { backgroundColor: opt.bg, color: opt.fg }
                                    : undefined
                                }
                                className={cn(
                              ...
                              >
                                ...
                                <span className="mt-1 text-[10px] leading-tight">{opt.short}</span>
                                {isSelected ? (
                                  <span className="absolute right-1 top-1 inline-flex h-4 w-4 items-center justify-center rounded-full bg-black/70 text-white">
                                    <Check className="h-2.5 w-2.5" aria-hidden />
                                  </span>
