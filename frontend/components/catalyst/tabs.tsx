...
export function CatalystTabsList({
  children,
  className,
}: {
  children: ReactNode;
  ...
  return (
    <TabList
      className={clsx(
        'inline-flex gap-1 rounded-xl border border-border bg-secondary p-1',
        className,
      )}
    >
      {children}
...
export function CatalystTabsTrigger({
  children,
  disabled = false,
}: {
  children: ReactNode;
...
export function CatalystTabsPanel({
  children,
  className,
}: {
  children: ReactNode;
  ...
  return (
    <TabPanel unmount={false} className={clsx('outline-none', className)}>
      {children}
    </TabPanel>
  );
}
