...
function applyFilter(
  ...
  if (filter.type === 'parti') {
    return items.filter(
      (item) => item.feed_party_tags?.includes(filter.slug) ?? false,
    );
  }
...
function sortItems(items: PostFeedItemView[], sortMode: FeedSortMode) {
  ...
  sorted.sort((a, b) => {
    if (sortMode === 'recent') {
      return (
        new Date(b.last_activity_at ?? b.open_at ?? 0).getTime() -
        new Date(a.last_activity_at ?? a.open_at ?? 0).getTime()
      );
    }

    return Number(
      (b.editorial_feed_score ?? 0) - (a.editorial_feed_score ?? 0),
    );
  });
