...
function isActive(filter: CategoryFilter, current: CategoryFilter): boolean {
  ...
  if (filter.type === 'sondage' && current.type === 'sondage')
    return filter.status === current.status;
  if (filter.type === 'politique' && current.type === 'politique')
    return filter.blocSlug === current.blocSlug;
  if (filter.type === 'parti' && current.type === 'parti')
    return filter.slug === current.slug;
  if (filter.type === 'subject' && current.type === 'subject')
    return filter.slug === current.slug;
  return false;
}
...
function FilterButton({
  ...
  return (
    <button
      type="button"
      onClick={() => {
        onFilterChange(active ? null : filter);
      }}
      className={cn(
