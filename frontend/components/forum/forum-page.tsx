...
function updateCommentBody(
  ...
  return tree.map((node) => {
    ...
    return {
      ...node,
      children: updateCommentBody(node.children, commentId, body),
    };
  });
}
...
function countComments(tree: CommentTreeNode[]): number {
  return tree.reduce(
    (total, node) => total + 1 + countComments(node.children),
    0,
  );
}
...
export function ForumPage({
  ...
  async function handleEditSubmit(payload: {
    ...
    setTree((previous) =>
      updateCommentBody(previous, payload.commentId, payload.body),
    );
  }
  ...
  return (
    <div className="grid gap-4">
      <section className="space-y-4" id="post-main">
        ...
        <PostToolbar sortMode={sortMode} onSortChange={setSortMode} />
        ...
        <CommentThread
          ...
          redirectPath={redirectPath}
          onReplySubmit={(draft) =>
            handleReplySubmit({ targetId: draft.targetId, body: draft.body })
          }
          onEditSubmit={(draft) =>
            handleEditSubmit({ commentId: draft.commentId, body: draft.body })
          }
