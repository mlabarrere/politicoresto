...
export async function setUsernameAction(formData: FormData) {
  ...
  if (usernameError) {
    log.warn(
      { event: 'setUsername.validation_failed', usernameError },
      'username invalid',
    );
  ...
  if (!userId) {
    log.error(
      { event: 'setUsername.unauthenticated' },
  ...
  if (duplicateResult.data) {
    log.warn(
      { event: 'setUsername.duplicate', user_id: userId, username },
...
export async function upsertAccountIdentityAction(formData: FormData) {
  const redirectPath = safeRedirectPath(
    String(formData.get('redirect_path') ?? '/me').trim() || '/me',
  );
  const displayName = String(formData.get('display_name') ?? '').trim();
  const usernameInput = String(formData.get('username') ?? '').trim();
  const bio = String(formData.get('bio') ?? '').trim() || null;
  const avatarUrl = String(formData.get('avatar_url') ?? '').trim() || null;
  const isPublicProfileEnabled =
    String(formData.get('is_public_profile_enabled') ?? 'on') === 'on';
...
export async function upsertPrivateProfileAction(formData: FormData) {
  const interestRaw = String(
    formData.get('political_interest_level') ?? '',
  ).trim();
  const notes = String(formData.get('notes_private') ?? '').trim() || null;
  const socioProfessionalCategory =
    String(formData.get('socio_professional_category') ?? '').trim() || null;
  const employmentStatus =
    String(formData.get('employment_status') ?? '').trim() || null;
  const educationLevel =
    String(formData.get('education_level') ?? '').trim() || null;
  const redirectPath = safeRedirectPath(
    String(formData.get('redirect_path') ?? '/me').trim() || '/me',
  );
