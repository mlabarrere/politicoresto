...
export async function getPublicProfile(username: string) {
  ...
  const [postsResult, commentsResult, deltasResult] = await Promise.all([
    supabase
      .from("v_thread_posts")
    ...
    supabase
      ...
      .order("created_at", { ascending: false })
      .limit(20),
    supabase.rpc("count_deltas_received", { p_user_id: userId })
  ]);
  ...
  const deltasReceived =
    typeof deltasResult.data === "number" ? deltasResult.data : 0;
