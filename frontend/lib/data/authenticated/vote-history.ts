...
function isCapabilityMissing(error: { message?: string; code?: string } | null | undefined) {
...
export async function getVoteHistoryEditorData(): Promise<VoteHistoryEditorData> {
  ...
  const [electionResult, resultRows, votesResult] = await Promise.all([
    ...
    supabase
      ...
      .select("id, election_id, rank, candidate_name, list_label, party_slug, nuance, pct_exprimes")
  ...
  if (electionResult.error) {
    ...
    if (isCapabilityMissing(electionResult.error)) {
      return { elections: [], votesByElectionId: {}, status: "unavailable", message: "Historique electoral non deploye sur cet environnement." };
    }
    return { elections: [], votesByElectionId: {}, status: "error", message: "Impossible de charger les scrutins." };
  }
  ...
  if (resultRows.error) {
    ...
    if (isCapabilityMissing(resultRows.error)) {
      return { elections: [], votesByElectionId: {}, status: "unavailable", message: "Resultats electoraux indisponibles." };
    }
    return { elections: [], votesByElectionId: {}, status: "error", message: "Impossible de charger les resultats." };
  }

  const resultsByElection = new Map<string, ElectionResultRow[]>();
  for (const r of (resultRows.data ?? []) as Array<ElectionResultRow & { election_id: string }>) {
  ...
  const elections: ElectionRow[] = ((electionResult.data ?? []) as ElectionRow[]).map((e) => ({
    ...e,
